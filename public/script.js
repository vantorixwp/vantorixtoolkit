...
(function () {
  ...
  if (gateKey && gateBtn) {
    ...
    function gateSay(text, kind) {
      ...
      gateStatus.className = 'gate__status' + (kind ? ' is-' + kind : '');
    }

    function requestDownload() {
      var key = gateKey.value.trim();

      if (!key) {
        gateSay('Enter the licence key we sent you.', 'error');
        gateKey.focus();
        return;
      }
      ...
      fetch('/api/download', {
        ...
        .then(function (response) {
          if (!response.ok) {
            return response.json().then(function (data) {
              throw new Error(data && data.message ? data.message : 'That key was not accepted.');
            });
          }
        ...
        .then(function (blob) {
          ...
          link.href = url;
          link.download = 'vantorix-pro.zip';
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
          URL.revokeObjectURL(url);
          gateSay('Key accepted — your download has started.', 'ok');
        })
        .catch(function (err) {
          gateSay(err.message || 'Something went wrong. Please message us.', 'error');
        })
        .then(function () {
          busy = false;
    ...
    gateBtn.addEventListener('click', requestDownload);

    gateKey.addEventListener('keydown', function (event) {
      if (event.key === 'Enter') {
